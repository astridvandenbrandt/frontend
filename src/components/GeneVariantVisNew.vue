...
export default {
  ...
  methods: {
    ...
    renderVis() {
      ...
      // Change color based on VR
      d3.select("#selectButtonAccessionData").on("change", function() {
        var selectedVR = d3.select("#selectButtonAccessionData").node().value;
        const selVR = selectedVR.split("_")
        const selectedVisRef = selVR[1]+"_"+selVR[2]
        console.log("selected VR: ", selectedVR, selectedVisRef);
